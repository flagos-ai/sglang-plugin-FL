...
class TestRunner:
    ...
    def _discover_e2e_tests(self) -> list[TestCase]:
        ...
        # Keep existing flat e2e/config tests runnable while the structured e2e tree is added later.
        for path in ("tests/test_e2e_config.py", "tests/test_precision_align.py"):
            if (_REPO_ROOT / path).exists() and self.task in (None, "config", "precision"):
                cases.append(
                    TestCase(
                        name=Path(path).stem,
                        pytest_path=path,
