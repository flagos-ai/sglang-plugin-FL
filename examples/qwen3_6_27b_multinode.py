...
Usage:
  ...
  Step 1 — Start master on node 0 (e.g. 192.168.0.66):
    python examples/qwen3_6_27b_multinode.py --role master --tp 8

  Step 2 — Start worker on node 1 (e.g. 192.168.0.65):
    python examples/qwen3_6_27b_multinode.py --role worker --tp 8

  NOTE: Start master FIRST, then start worker within a few minutes.

Full tested command (2 nodes × 4 GPUs each, TP=8):
