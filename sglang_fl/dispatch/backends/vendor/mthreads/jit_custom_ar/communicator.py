...
class MusaJitCustomAllreduce:
    _SUPPORTED_WORLD_SIZES = [2, 4, 6, 8]
    # Keep the broad 512 MiB default for other users/shapes.  The
    # Qwen3.6-35B-A3B 16K/TP2 experiment opts into a smaller value through the
    # factory below so this optimization cannot silently shrink unrelated
    # custom-AR inputs.
    _MAX_CAR_SIZE = 512 * 1024 * 1024
    _RANK_DATA_WIDTH = 8
    _RANK_DATA_ELEMENT_SIZE = ctypes.sizeof(ctypes.c_int64)
    _RANK_DATA_SLOT_BYTES = _RANK_DATA_WIDTH * _RANK_DATA_ELEMENT_SIZE
    requires_graph_capture_registration_recapture = False

    @classmethod
    def _graph_rank_data_slot_capacity(
        cls, max_size: int, graph_registered_input_enabled: bool
    ) -> int:
        ...
        # The compatibility path does not consume graph-registered slots. Keep
        # one 64-byte, non-empty shape as an allocation invariant; disabled
        # graph-registration paths must not access this storage.
        if not graph_registered_input_enabled:
            return 1
        if max_size < cls._RANK_DATA_SLOT_BYTES:
            raise ValueError(
                "MUSA JIT custom allreduce max_size must fit at least one "
                "rank-data slot (64 bytes)."
            )
        return max_size // cls._RANK_DATA_SLOT_BYTES
    ...
    def __init__(
        ...
        graph_rank_data_slot_capacity = self._graph_rank_data_slot_capacity(
            max_size, self._graph_registered_input_enabled
        )
        self._graph_rank_data_slot_count = graph_rank_data_slot_capacity
        self._graph_rank_data_bytes = (
            graph_rank_data_slot_capacity * self._RANK_DATA_SLOT_BYTES
        )
        ...
        logger.info(
            "MUSA JIT custom allreduce graph rank-data allocation: "
            "enabled=%s slot_count=%d bytes=%d",
            self._graph_registered_input_enabled,
            self._graph_rank_data_slot_count,
            self._graph_rank_data_bytes,
        )
...
_MUSA_CUSTOM_AR_MAX_SIZE_MB_ENV = "SGLANG_MUSA_CUSTOM_AR_MAX_SIZE_MB"
_MUSA_CUSTOM_AR_MIN_SIZE_MB = 1
_MUSA_CUSTOM_AR_MAX_SIZE_MB = 512


def _musa_custom_ar_max_size_from_env() -> Optional[int]:
    """Return an explicit MUSA custom-AR workspace bound, if configured.

    The class default intentionally remains broad because this communicator is
    also used by workloads whose payloads exceed the Qwen3.6 16K bound.  The
    smaller workspace is therefore an opt-in factory setting.  Parse the
    environment without trimming or accepting signs/decimal/Unicode digits so
    malformed values fail before a communicator allocates either workspace.
    """

    raw = os.environ.get(_MUSA_CUSTOM_AR_MAX_SIZE_MB_ENV)
    if raw is None:
        return None
    if not raw.isascii() or not raw.isdecimal():
        raise ValueError(
            f"{_MUSA_CUSTOM_AR_MAX_SIZE_MB_ENV} must be an integer MiB value "
            f"in [{_MUSA_CUSTOM_AR_MIN_SIZE_MB}, {_MUSA_CUSTOM_AR_MAX_SIZE_MB}]"
        )
    value_mb = int(raw)
    if not (_MUSA_CUSTOM_AR_MIN_SIZE_MB <= value_mb <= _MUSA_CUSTOM_AR_MAX_SIZE_MB):
        raise ValueError(
            f"{_MUSA_CUSTOM_AR_MAX_SIZE_MB_ENV} must be an integer MiB value "
            f"in [{_MUSA_CUSTOM_AR_MIN_SIZE_MB}, {_MUSA_CUSTOM_AR_MAX_SIZE_MB}]"
        )

    max_size = value_mb * 1024 * 1024
    slot_bytes = MusaJitCustomAllreduce._RANK_DATA_SLOT_BYTES
    if max_size < slot_bytes or max_size % slot_bytes != 0:
        raise ValueError(
            f"{_MUSA_CUSTOM_AR_MAX_SIZE_MB_ENV} produces max_size={max_size}, "
            f"which must be aligned to the {slot_bytes}-byte rank-data slot"
        )
    return max_size


def musa_custom_allreduce_factory():
    """Return the MUSA custom-AR constructor with its validated workspace.

    An absent environment variable returns the class itself, preserving the
    communicator's 512 MiB default.  An explicit value returns a partial with
    the validated byte bound.  Logging the effective bytes makes the selected
    capacity visible in startup provenance without logging arbitrary input.
    """

    max_size = _musa_custom_ar_max_size_from_env()
    if max_size is None:
        max_size = MusaJitCustomAllreduce._MAX_CAR_SIZE
        source = "class-default"
        constructor = MusaJitCustomAllreduce
    else:
        source = f"env:{_MUSA_CUSTOM_AR_MAX_SIZE_MB_ENV}"
        constructor = partial(MusaJitCustomAllreduce, max_size=max_size)
    logger.info(
        "MUSA JIT custom allreduce workspace max_size_bytes=%d source=%s",
        max_size,
        source,
    )
    return constructor
...
def dispatch_custom_allreduce():
    ...
    if _use_jit_all_reduce():
        ...
        if _is_musa:
            ...
            return musa_custom_allreduce_factory()
